...
void generate_elf(std::ofstream &f, std::vector<uint8_t> &output_buffer, uint64_t data_size, uint64_t bss_size) {
	...
	ehdr.shnum = 5 + !!data_size + !!bss_size; // 5 for null, text, symtab, strtab, shstrtab
	ehdr.shstrndx = ehdr.shnum - 1; // last section for section names
	...
	if (data_size) {
		...
		shdr.offset = shdr.offset + ((output_buffer.size() - data_size + page_size - 1) & ~(page_size - 1));
		shdr.size = data_size;
		shdr.link = 0;
		shdr.info = 0;
		shdr.addralign = 4;
...
void generate_object(std::ofstream &f, std::vector<uint8_t> &output_buffer, uint64_t data_size, uint64_t bss_size) {
	// structure:
	//  ELF header
	//  program headers
	//  section headers
	...
	ehdr.phnum = 0;
	ehdr.shentsize = sizeof(section_header);
	ehdr.shnum = 5 + !!data_size + !!bss_size + !rela_relocations.empty(); // 5 for null, text, symtab, strtab, shstrtab
	...
	shdr.addr = 0;
	shdr.offset = shdr.offset + shdr.size;
	shdr.size = (reloc_table.size() + data_labels.size() + bss_labels.size() + rela_relocations.size() + 1) * sizeof(symbol);
	shdr.link = ehdr.shnum - 2; // strtab
	...
	shdr.addr = 0;
	shdr.offset = shdr.offset + shdr.size;
	shdr.size = rela_relocations.size() * sizeof(relocation);
	...
	std::unordered_map<std::string, size_t> symtab;
	for (auto s : reloc_table) {
		if (global.find(s.first) != global.end())
			continue;
		sym.name = i;
		i += s.first.size() + 1;
		sym.info = 0; // local
	...
	reloc.addend = -4;
	// write rela.text
	for (auto &r : rela_relocations) {
		reloc.offset = r - data_size;
		reloc.info = *(uint32_t *)(output_buffer.data() + r) + 1;
		reloc.info = (reloc.info << 32) | 4;
